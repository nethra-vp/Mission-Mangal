/*
 * Scene 4: Mission Control - Launch Countdown
 * Duration: Variable (demonstrates full countdown sequence)
 * Extracted from timer.c - COMPLETE VERSION
 */

#ifndef SCENE_4_TIMER_H
#define SCENE_4_TIMER_H
...
static float s4_gTime = 0.0f;

static void s4_setColor3(float r, float g, float b) {
	GLfloat mat[4] = {r, g, b, 1.0f};
	glColor3f(r, g, b);
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, mat);
}

static void s4_drawBox(float sx, float sy, float sz) {
	glPushMatrix();
	glScalef(sx, sy, sz);
	glutSolidCube(1.0f);
	glPopMatrix();
...
static void s4_drawBitmapText(const char *txt, void *font) {
	int i;
	for (i = 0; txt[i] != '\0'; ++i) {
...
static void s4_drawSevenSegDigitGlyph(float x0, float yBase, float dW, float dH, float thick, int digit, float pulse, float alpha) {
...
static void s4_drawVideoWallMainContent(void) {
	int i;
	int totalSec;
	int hours;
	int minutes;
	...
	float secPhase;
	float approach;
	float pulse = 0.5f + 0.5f * sinf(s4_gTime * 7.0f);
	int digits[8];
	...
	/* Countdown from 10 to 0 seconds, then holds at 0. */
	loopCountdown = 10.0f - s4_gTime;
	...
	for (i = 0; i <= 40; ++i) {
		...
		float y = 1.45f + sinf(t * 4.8f + s4_gTime * 0.7f) * 0.18f;
	...
	for (i = 0; i < 9; ++i) {
		float level = 0.2f + (0.5f + 0.5f * sinf(s4_gTime * 4.5f + i * 0.8f)) * 1.1f;
	...
	for (i = 0; i < 8; ++i) {
		...
		s4_drawSevenSegDigitGlyph(x0, yBase, dW, dH, thick, digits[i], pulse, 1.0f);
		...
		if (i == 6 || i == 7) {
			...
			s4_drawSevenSegDigitGlyph(x0 - 0.06f * (1.0f - approach), yBase, dW, dH, thick, dPrev, pulse, ghostOut);
			s4_drawSevenSegDigitGlyph(x0 + 0.02f * sinf(s4_gTime * 40.0f), yBase, dW, dH, thick, dNow, pulse, 0.78f);
			s4_drawSevenSegDigitGlyph(x0 + 0.06f * approach, yBase, dW, dH, thick, dNext, pulse, ghostIn);
		}
	...
	s4_drawBitmapText("T-MINUS COUNTDOWN", GLUT_BITMAP_HELVETICA_18);
	glRasterPos3f(-2.1f, -1.72f, 0.5f);
	s4_drawBitmapText("STATUS: GO FOR LAUNCH", GLUT_BITMAP_HELVETICA_18);

	glEnable(GL_LIGHTING);
}

static void s4_drawVideoWall(void) {
	s4_setColor3(0.12f, 0.12f, 0.16f);
	glPushMatrix();
	glTranslatef(0.0f, 5.5f, -31.0f);
	s4_drawBox(42.0f, 14.0f, 0.8f);

	glPushMatrix();
	glTranslatef(0.0f, 0.0f, 0.42f);
	glScalef(2.0f, 2.0f, 1.0f);
	s4_drawVideoWallMainContent();
	glPopMatrix();

	glPopMatrix();
}

static void s4_drawScientist(float x, float y, float z, float r, float g, float b, int gesture) {
	float arm = 0.35f + 0.15f * sinf(s4_gTime * 2.0f + x);
	glPushMatrix();
	glTranslatef(x, y, z);
	...
	s4_drawBox(0.32f, 0.72f, 0.2f);
	glPopMatrix();

	s4_setColor3(0.92f, 0.76f, 0.62f);
	glPushMatrix();
	glTranslatef(0.0f, 1.02f, 0.0f);
	glutSolidSphere(0.12f, 14, 14);
	glPopMatrix();

	s4_setColor3(0.12f, 0.12f, 0.14f);
	glPushMatrix();
	glTranslatef(0.0f, 1.14f, 0.0f);
	s4_drawBox(0.22f, 0.08f, 0.22f);
	glPopMatrix();

	s4_setColor3(r * 0.8f, g * 0.8f, b * 0.8f);
	glPushMatrix();
	glTranslatef(-0.18f, 0.65f + (gesture ? arm : 0.0f), 0.0f);
	s4_drawBox(0.07f, 0.45f, 0.07f);
	glPopMatrix();
...
static void s4_drawDeskStation(float x, float y, float z, float rotY, const char *dept) {
	glPushMatrix();
	glTranslatef(x, y, z);
	glRotatef(rotY, 0.0f, 1.0f, 0.0f);

	s4_setColor3(0.18f, 0.19f, 0.22f);
	glPushMatrix();
	glTranslatef(0.0f, 0.35f, 0.0f);
	s4_drawBox(1.3f, 0.08f, 0.6f);
	glPopMatrix();

	s4_setColor3(0.14f, 0.14f, 0.18f);
	glPushMatrix();
	glTranslatef(0.0f, 0.15f, 0.0f);
	s4_drawBox(1.25f, 0.25f, 0.54f);
	glPopMatrix();
	...
	s4_drawBitmapText(dept, GLUT_BITMAP_8_BY_13);
	glEnable(GL_LIGHTING);

	s4_setColor3(0.08f, 0.08f, 0.1f);
	glPushMatrix();
	glTranslatef(0.0f, -0.06f, -0.18f);
	s4_drawBox(0.6f, 0.28f, 0.5f);
	glPopMatrix();

	glPopMatrix();
}

static void s4_drawTieredControlGallery(void) {
	int row, seat;
	float baseZ = -4.0f;
	...
	for (row = 0; row < 4; ++row) {
		...
		s4_setColor3(0.16f, 0.17f, 0.2f);
		glPushMatrix();
		glTranslatef(0.0f, y - 0.2f, z - 0.5f);
		s4_drawBox(22.0f, 0.35f, 3.0f);
		glPopMatrix();

		/* Department names removed as requested */
		/*
		glDisable(GL_LIGHTING);
        glColor3f(0.0f, 0.8f, 1.0f);
        glRasterPos3f(-10.5f, y + 1.0f, z); 
        s4_drawBitmapText(deptNames[row % 3], GLUT_BITMAP_HELVETICA_18);
        glEnable(GL_LIGHTING);
		*/
		...
		for (seat = -4; seat <= 4; ++seat) {
			...
			float zz = z;
			float rot = 0.0f;
			s4_drawDeskStation(x, y, zz, rot, "");
		}
	}

	// Row 0 (Z = -4.0)
    s4_drawScientist(-0.6f, 0.2f, -4.5f, 0.20f, 0.33f, 0.67f, 1);
    s4_drawScientist(7.2f, 0.2f, -4.5f, 0.1f, 0.5f, 0.2f, 1);

    // Row 1 (Z = -7.5)
    s4_drawScientist(-5.0f, 0.95f, -8.0f, 0.23f, 0.23f, 0.54f, 0);
    s4_drawScientist(4.0f, 0.95f, -8.0f, 0.65f, 0.40f, 0.20f, 0);
    s4_drawScientist(-6.5f, 0.95f, -8.0f, 0.23f, 0.23f, 0.54f, 0);

    // Row 2 (Z = -11.0)
    s4_drawScientist(2.5f, 1.70f, -11.5f, 0.8f, 0.2f, 0.1f, 0);
    s4_drawScientist(-8.0f, 1.7f, -11.5f, 0.9f, 0.9f, 0.9f, 0);
    s4_drawScientist(-3.5f, 1.7f, -11.5f, 0.2f, 0.6f, 0.3f, 1);
    s4_drawScientist(6.0f, 1.7f, -11.5f, 0.3f, 0.3f, 0.8f, 0); 

    // Row 3 (Z = -14.5)
    s4_drawScientist(9.0f, 2.45f, -15.0f, 0.15f, 0.15f, 0.15f, 0);
    s4_drawScientist(-4.8f, 2.45f, -15.0f, 0.88f, 0.88f, 0.9f, 0);
    s4_drawScientist(8.5f, 2.45f, -15.0f, 0.7f, 0.4f, 0.2f, 0);
}

static void s4_drawServerRack(float x, float y, float z, float phase) {
	int i;
	glPushMatrix();
	glTranslatef(x, y, z);

	s4_setColor3(0.14f, 0.18f, 0.22f);
	s4_drawBox(1.35f, 5.8f, 1.15f);

	glDisable(GL_LIGHTING);
	for (i = 0; i < 14; ++i) {
		float led = 0.35f + 0.65f * (0.5f + 0.5f * sinf(s4_gTime * 4.5f + i + phase));
...
static void s4_drawServerGallery(void) {
	int c, r;

	glDisable(GL_LIGHTING);
	glColor4f(0.55f, 0.72f, 0.9f, 0.18f);  /* This was the light blue background panel */
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	for (r = 0; r < 3; ++r) {
		for (c = 0; c < 9; ++c) {
			...
			s4_drawServerRack(x, 1.8f, z, (float)(c + r * 2));
		}
	}
}

static void s4_drawArchitecture(void) {
	s4_setColor3(0.09f, 0.1f, 0.12f);
	glPushMatrix();
	glTranslatef(0.0f, -1.5f, -14.0f);
	s4_drawBox(44.0f, 0.3f, 48.0f);
	glPopMatrix();

	s4_setColor3(0.08f, 0.09f, 0.11f);
	glPushMatrix();
	glTranslatef(-22.0f, 3.25f, -14.0f);
	s4_drawBox(0.4f, 10.0f, 48.0f);
	glPopMatrix();

	s4_setColor3(0.08f, 0.09f, 0.11f);
	glPushMatrix();
	glTranslatef(-22.0f, 5.2f, -14.0f);
	s4_drawBox(0.4f, 14.0f, 48.0f);
	glPopMatrix();
...
static void s4_setupLighting(void) {
...
static void scene4_display(void) {
	...
	s4_drawArchitecture();
	s4_drawVideoWall();
	s4_drawTieredControlGallery();
	s4_drawServerGallery();

	glutSwapBuffers();
}

static void scene4_update(float dt) {
	s4_gTime += dt;
}
